...
fid=fopen('ONIY.tenv3.txt');
 ...
 %
   figure(1),clf,
    subplot(311),plot(t1,x1-x1(1),t2,x2-x2(1),t3,x3-x3(1)),ylabel('east (m)')
      legend('ONIY','MRSI','ANTP','location','northwest')
 ...
 %
 %calculate the velocity componenet
 %calculate the horizontal velocity of each station over the full
 %timeseries
 %
  ...
  %
  %Plot the velocity vector on a map
  %Load the coastline data & political boundary data
  %
...
%
  ...
  %Visualize & think about the strain results
  %
